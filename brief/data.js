...
const BRIEF_DATA = {
  stats: { skills: 13, tests: 216, demos: 16 },
  ...
  install: [
    ...
    { title: "Any agent with a shell", text: "git clone github.com/wjlgatech/FDE-os && make check — one finish line for humans, CI, and agents alike (216 tests, offline, stdlib-only)." },
  ...
  features: [
    ...
    },
    {
      ...
      examples: [
        { cmd: "python3 take-home/enterprise-agentic-triage/scripts/run.py --out runs/latest",
        ...
        { cmd: "python3 take-home/enterprise-agentic-triage/scripts/eval.py --run-dir runs/latest",
  ...
  contract: [
    ...
    { k: "Exit codes gate CI", v: "PASS/BLOCK, GO/NO-GO, drift, zero-match — every verdict is an exit code, so the same command a human runs is the command CI enforces. make check = 216 tests, offline, no keys." },
  ...
  corpus: [
    "WHAT /FDE-os IS: the master tool over the FDE-os toolkit — one front door (skills/fde-os/SKILL.md) that routes any forward-deployed-engineering ask to the right engine. The repo is simultaneously a Claude Code plugin (marketplace add wjlgatech/FDE-os), a Codex plugin, a clonable toolkit gated by make check (216 offline stdlib tests), and an MCP server (8 tools over stdio JSON-RPC). 13 skills, 2 MCP servers, a compiled external-tooling hub, and a reference agent runtime.",
    ...
    "KNOWLEDGE COMPILERS DEMO (real outputs): knowledgefy built the Snowflake Summit vault into a graph — 16 concepts, 17 evidence nodes, 32 edges, every evidence node provenance-pinned (no citation, no node). jd-compiler compiled the enterprise-class agentic-AI JD into clusters: languages (python), agent frameworks (langchain, langgraph), vector DBs (milvus, pinecone, weaviate), eval-obs (evaluation, observability, tracing), patterns (multi-agent, orchestration). That same JD was later answered with the built take-home.",
    ...
    "REFERENCE RUNTIME DEMO (real outputs): the triage agent runs 12 procurement requests through a durable graph (checkpoint per step, human-gate interrupts, retries, bounded self-correction) — 3 approvals, 1 deny, 7 escalations each routed to the right human, 1 invalid. Its eval grades against labeled ground truth: decision_accuracy 1.0, escalation_recall 1.0 (gated at exactly 1.0 — one missed escalation fails the build), citation_doc_coverage 1.0, citation_validity 1.0 (every cited quote exists verbatim in the policy corpus), trace_completeness 1.0. VERDICT PASS, exit 0. Presented in full at enterprise-triage-brief.vercel.app.",
    "THE GOLDEN DATASET (how well does it actually work): 47 labeled cases across 11 tag families (common, boundary, tier, PII, supersede, memory, injection incl. a false-positive probe, invalid, competitive-bids, combo) grade the runtime end to end. The two-phase story is the point: the FIRST run was an honest FAIL exit 2 (escalation recall 0.889) — the golden set exposed an unimplemented policy rule (v3 §3 competitive bids above $75k, Preferred exempt) and a crash on vendor-less invalid requests. Both were fixed the same day (the bids threshold is parsed from retrieved policy text like every other rule); the second run scored 1.0 on all five metrics, 47/47, with the original 12-case set unregressed. The golden run is now a permanent CI gate — a dataset that cannot fail you is not measuring anything.",
