...
| `take-home/` | **Interview-grade end-to-end builds** — each dir is a realistic take-home answered head-to-head against a real JD, with its own eval gate. `enterprise-agentic-triage/` = the six JD "thinking layer" terms as one durable agent graph (checkpoints, HITL interrupt/resume, hybrid retrieval + verifiable citations, session memory, budget-receipted context, monotonic guardrails, task-based eval vs ground truth — 59 labeled cases: the original 12 + a 47-case golden set (`data/golden/`, 11 tag families) whose first run honestly failed and exposed 2 real defects, now a CI gate via `test_golden_eval.py`; playable live at https://enterprise-triage-brief.vercel.app/#play against the real `/api/triage`). CI discovers `take-home/*/tests` (same finish line as skills). Its `graph.py`/`tools.py`/`retrieval.py`/`memory.py` are candidate cores for future skills — import, don't re-implement. |
...
| `brief/` | The **/FDE-os master-tool brief** (live: https://fde-os-master.vercel.app) — the agentic webapp presenting the master skill: 14 real, CI-verified demos (`data.js` is the single source for UI *and* copilot corpus; `api/chat.js` is corpus-restricted with a labeled offline fallback). Outputs shown are captured runs; the same commands are enforced by `skills/fde-os/tests/test_master_examples.py`. Redeploy: `cd brief && vercel deploy --prod --yes`. |
